...
//                                $output = str_replace('0', '', $month_only);
                                //echo $output;
//                                $OrderNumber = $this->session->userdata('dealercode') . "/" . $OrderType['Code'] . "T" . "/" . $output;
...
//                        }
                    ...
                    <div>
                        <table id="uo" title="Urgent Orders" style="width:auto;height:auto;"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th field="PartNumber" width="60" editor="{type:'combobox',options:{valueField:'PartNumber', textField:'PartNumber', url:'<?= base_url() ?>/index.php/invoices/parts',required:true}}">Part Number</th>
                        ...
                        <div id="toolbar">
                            ...
                            <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="javascript:$('#uo').edatagrid('addRow');
                            <?php
                            ...
                            <a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="javascript:$('#uo').edatagrid('destroyRow');javascript:$('#uo').edatagrid('reload')">Delete</a>
                            <a href="#" id="saveURL" class="easyui-linkbutton" iconCls="icon-save" plain="true" onclick="javascript:$('#uo').edatagrid('saveRow')">Save</a>
                            <a href="#" class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="javascript:$('#uo').edatagrid('cancelRow')">Cancel</a>
                        </div>
                    </div>
                </fieldset>
            </form>
...
<script>
    ...
    $('#uo').edatagrid({
        url: '<?= base_url() ?>/index.php/urgentorder/allUrgentOrders',
        saveUrl: '<?= base_url() ?>/index.php/urgentorder/saveUrgentOrder',
        updateUrl: '<?= base_url() ?>/index.php/parts/edit',
        destroyUrl: '<?= base_url() ?>/index.php/urgentorder/delete',
    ...
    function destroyUser() {
        var row = $('#uo').edatagrid('getSelections');
        $.each(row, function (index, value) {
            if (value) {
                $.messager.confirm('Confirm', 'Are you sure you want to delete this order?', function (r) {
                    if (r) {
                        $.post('<?= base_url() ?>/index.php/urgentorder/delete', {id: value.OrderNumber}, function (result) {
                            window.res = result;
                            if (result.success) {
                                $('#uo').edatagrid('reload');    // reload the user data
