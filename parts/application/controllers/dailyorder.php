...
class Dailyorder extends CI_Controller {
...
//                    print_r($Data["Part"]);exit();
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->DailyOrderReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->DailyOrderReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->DailyOrderReport(NULL, $Brand);
                }
            }
//            else if ($Filter == "Both") {
//                $Data = $Reporting->DailyOrderReport($OrderNumber, $Brand);
//            }
//            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
            ...
            $pdf->SetTitle('Daily Order Reports');
            $pdf->SetSubject('');
            $pdf->SetKeywords('Daily Order Report');

            //set default header data
//        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Parts Division', "Daily Order Report");
...
//        $pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
    ...
    function parts() {
        $invoice = new Parts_invoices();
        return $invoice->fillPartCombo('Daily Order');
...
//        echo json_encode($fillPartCombo);
    ...
    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneDailyOrder($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteDailyOrder = $invoice->deleteDailyOrder($idOrderNumber["idOrderNumber"]);
            if ($deleteDailyOrder) {
    ...
    function saveDailyOrder() {
        ...
        redirect(base_url() . "/index.php/dailyorder/index");
    }
