                            ...
                            <tbody id="finalResult">
                                ...
                                foreach ($Sale as $row) {
                                    ...
                                    <tr id="carUsers">
                                        ...
                                        <td class="tbl-name"><?= $row['TotalPrice'] - $row['Discount'] +$row['Surcharge']?></td>
                                        ...
                                        <td class="tbl-name"><a href="<?= base_url() . 'index.php/sales/generate1/' . $row['idSale'] ?>">Generate</a></td>
                ...
                if (a.length > 0) {
                    try {
                        ...
                        $.each(a, function (i, val) {
                            ...
                            <td class='tbl - name'>" + val.PartNumber + " (" + val.PartName + ") </td>\n\
<td class='tbl - name'>" + val.Name + " (" + val.Name + ") </td>\n\
                            <td>" + val.SaleType + "</td>\n\
...
<td>" + val.TotalPrice + "</td><td>" + val.CostPrice + "</td><td>" + val.TotalCost + "</td><td>" + val.SaleDate + "</td>";
							...
							var url = '<?php echo base_url("index.php/sales/generate/")?>';
							var url1 = '<?php echo base_url("index.php/sales/generate1/")?>';
                            items += "<td class='tbl-name'><a href='"+url+"/"+val.SaleId+"'>"+ 'Generate' + "</a></td>";
                            items += "<td class='tbl-name'><a href='"+url1+"/"+val.SaleId+"'>"+ 'Generate' + "</a></td>";
							items += "<td class='tbl-name' onclick='saleReturn("+JSON.stringify(val)+")'>" + 'Generate' +"</td>";
							items +=  "</tr>";
                        });
                    ...
                    $("#finalResult").html("<td style='border: 0px' colspan='14'><span style='font-weight: bold;'>Search Result:</span> '" + search + "' has no record.</td>");
                }
            }
        });
    });
