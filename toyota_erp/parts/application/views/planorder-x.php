        ...
        <div class="right-pnel">
            <form name="myform" onSubmit="return validationform()" method="post"
                ...
                <fieldset>
                    ...
                    <div>
                        <table id="po" title="Plan Orders" style="width:auto;height:auto;"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th field="PartNumber" width="60" editor="{type:'combobox',options:{valueField:'PartNumber', textField:'PartNumber', url:'<?= base_url() ?>/index.php/invoices/parts/<?= "Plan Order" ?>',required:true}}">Part Number</th>
                                    ...
                                    <th field="WSPrice" width="50" editor="{type:'validatebox',options:{required:false}}">Wholesale Price</th>
                                    <th field="TotalPrice" width="50" editor="{type:'validatebox',options:{required:false}}">Total Price</th>
...
<script>
    ...
    $('#po').edatagrid({
        url: '<?= base_url() ?>/index.php/planorder/allPlanOrders',
        saveUrl: '<?= base_url() ?>/index.php/planorder/savePlanOrder',
        updateUrl: '<?= base_url() ?>/index.php/parts/edit',
        destroyUrl: '<?= base_url() ?>/index.php/planorder/delete',
        ...
        $.each(row, function (index, value) {
            if (value) {
                $.messager.confirm('Confirm', 'Are you sure you want to delete this order?', function (r) {
                    if (r) {
                        $.post('<?= base_url() ?>/index.php/planorder/delete', {id: value.OrderNumber}, function (result) {
                            window.res = result;
                            if (result.success) {
                                $('#po').edatagrid('reload');    // reload the user data
