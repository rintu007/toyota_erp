...
class Byseaorder extends CI_Controller {
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->BySeaOrderReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->BySeaOrderReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->BySeaOrderReport(NULL, $Brand);
                }
            }
//            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
            ...
            $pdf->SetTitle('By Sea Order Reports');
            $pdf->SetSubject('');
            $pdf->SetKeywords('By Sea Order Report');

            //set default header data
//        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Parts Division', "By Sea Order Report");
            ...
            <thead>
                <tr>
                    ...
                    <td width="200" height="30" style="line-height: 30px;" align="center" colspan="2"><b>By Sea Price</b></td>
            ...
            <tbody>';
                ...
                <tr>
                    ...
                    <td width="200" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['BySeaPrice'] . '</td>
...
//        $pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
    ...
    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneBySeaOrder($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteBySeaOrder = $invoice->deleteBySeaOrder($idOrderNumber["idOrderNumber"]);
            if ($deleteBySeaOrder) {
...
//        echo json_encode($fillPartCombo);
    ...
    function allBySeaOrders() {
        $invoice = new Parts_invoices();
        $DailyOrder = $invoice->allBySeaOrders();
        echo json_encode($DailyOrder);
    }

    function saveBySeaOrder() {
        $invoice = new Parts_invoices();
        $savePlanOrder = $invoice->saveBySeaOrder();
        redirect(base_url() . "/index.php/byseaorder/index");
    }
