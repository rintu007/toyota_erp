...
class Warrantyorder extends CI_Controller {
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->WarrantyOrderReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->WarrantyOrderReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->WarrantyOrderReport(NULL, $Brand);
                }
            }
//            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
            ...
            $pdf->SetTitle('Warranty Order Reports');
            $pdf->SetSubject('');
            $pdf->SetKeywords('Warranty Order Report');

            //set default header data
//        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Parts Division', "Warranty Order Report");
            ...
            <thead>
                <tr>
                    ...
                    <td width="150" height="30" style="line-height: 30px;" align="center" colspan="2"><b>Model Code</b></td>
                    <td width="150" height="30" style="line-height: 30px;" align="center" colspan="2"><b>Frame #</b></td>
                    <td width="150" height="30" style="line-height: 30px;" align="center" colspan="2"><b>TR #</b></td>
            ...
            <tbody>';
                ...
                <tr>
                    ...
                    <td width="150" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['ModelCode'] . '</td>
                    <td width="150" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['FrameNo'] . '</td>
                    <td width="150" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['TrNo'] . '</td>
            ...
            <thead>
                <tr>
                    ...
                    <td width="130" height="30" style="line-height: 30px;" align="center" colspan="2"><b>Model Code</b></td>
                    <td width="130" height="30" style="line-height: 30px;" align="center" colspan="2"><b>Frame #</b></td>
                    <td width="130" height="30" style="line-height: 30px;" align="center" colspan="2"><b>Tr #</b></td>
            ...
            <tbody>';
                ...
                <tr>
                    ...
                    <td width="130" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['ModelCode'] . '</td>
                    <td width="130" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['FrameNo'] . '</td>
                    <td width="130" height="30" style="line-height: 30px;" align = "center" colspan="2">' . $allData['TrNo'] . '</td>
...
//        $pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
            ...
            //This method has several options, check the source code documentation for more information.
            $pdf->Output('TWM-PARTS-WARRANTY-ORDER-REPORT-' . date('d/m/Y') . '.pdf', 'I');
        }
    ...
    function view() {
        ...
        $Data['allWarrantyOrder'] = $invoice->allWarrantyOrder();
        $this->load->view('header_parts', $Data);
...
//        echo json_encode($fillPartCombo);
    ...
    function WarrantyOrder($idInvoice) {
        $Data = array();
        $invoice = new Parts_invoices();
        $Data['OrderType'] = $invoice->getWarrantyOrderCode()[0];
        $Data['DealerCode'] = $invoice->getDealerCode()[0];
        $Data['OrderNo'] = $invoice->getOrderNumber($invoice->getWarrantyOrderCode()[0]);
    ...
    function saveWarrantyOrder() {
        $invoice = new Parts_invoices();
        $saveWarrantyOrder = $invoice->saveWarrantyOrder();
        redirect(base_url() . "/index.php/Warrantyorder/index");
    }

    function deleteWarrantyOrder() {
        print_r($_REQUEST);
    }

    function allWarrantyorder() {
        $invoice = new Parts_invoices();
        $WarrantyOrder = $invoice->allWarrantyOrder();
        echo json_encode($WarrantyOrder);
    }

    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneWarrantyOrder($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteDailyOrder = $invoice->deleteWarrantyOrder($idOrderNumber["idOrderNumber"]);
            if ($deleteDailyOrder) {
    ...
    function orderNumber() {
        $invoice = new Parts_invoices();
        $orderNumber = $invoice->getOrderNumber($invoice->getWarrantyOrderCode()[0]);
        return $orderNumber;
    }
