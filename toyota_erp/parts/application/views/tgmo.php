<div id="wrapper">
    <div id="content">
        ...
        <div class="right-pnel">
            <form name="myform" onSubmit="return validationform()" method="post"
                  action="<?= base_url() ?>/index.php/ToyotaGenuineMotorOil/saveTgmo" class="form validate-form animated fadeIn">
                <fieldset>
                    ...
                    <div class="feildwrap">
                        <div class="btn-block-wrap">
                            <div>
                                ...
                                $OrderNum = $cookieData['Code'] . "-" .'TGMO' . "-" . $year_only."-". $month_only . "-" . $orderNumber;
                                ?>
                            </div>
                        </div>
                    </div>
 ...
 <th width="12%">Total LTR QTY</th>
                    ...
                    <div>
                        <br>
                        <input type="submit" class="btn" style="float: right;" value="Add TGMO">
                    </div>
                </fieldset>
            </form>
        </div>
...
<script>
	...
	var length = $(".tblPurchaseLocal").length+1;
        ...
        items += "<tr class='tblPurchaseLocal'><td class='tbl-count'>"+length+"</td><td class='tbl-part'><select onchange=getPart(this) class='chosen-select' name='idPart[]' style='background: #fff; padding: 7px 4px 7px 7px; border: 1px solid #dfdfdf; padding: 7px 10px; -webkit-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09); -moz-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09); box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09);' id='PartNumber'><option>Select Part Number</option><?php
                ...
                "<td class='tbl-price'><input type='text' name='Ltr[]'  onfocusout=total(this) style='width: 70px;background: #fff; padding: 7px 4px 7px 7px; border: 1px solid #dfdfdf; padding: 7px 10px; -webkit-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09); -moz-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09); box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09);' id='Price' placeholder='LTR'></td>" +
                "<td class='tbl-tlq'><input type='text' name='Tlq[]' style='width: 70px;background: #fff; padding: 7px 4px 7px 7px; border: 1px solid #dfdfdf; padding: 7px 10px; -webkit-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09); -moz-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09); box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.09);' id='Tlq' placeholder='Total Ltr Qty' readonly></td>" +
...
function total(Source){
    ...
    var ltr = $(Source).val();
    var qty = $(Source).parent().prev().find('input').val();
    var total = ltr * qty ;
    $(Source).parent().next().find('input').val(total);
}
