...
class VOR extends CI_Controller {
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->VORReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->VORReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->VORReport(NULL, $Brand);
                }
            }
//            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
            ...
            $pdf->SetTitle('VOR Reports');
            $pdf->SetSubject('');
            $pdf->SetKeywords('VOR Report');

            //set default header data
//        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Parts Division', "VOR Report");
...
//        echo json_encode($fillPartCombo);
    ...
    function saveVOR() {
        $invoice = new Parts_invoices();
        $saveDhamkapackage = $invoice->saveVOR();
        redirect(base_url() . "/index.php/VOR/index");
    }

    function allVOR() {
        $invoice = new Parts_invoices();
        $Dhamkapackage = $invoice->allVOR();
        echo json_encode($Dhamkapackage);
    }

    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneVOR($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteDailyOrder = $invoice->deleteVOR($idOrderNumber["idOrderNumber"]);
            if ($deleteDailyOrder) {
