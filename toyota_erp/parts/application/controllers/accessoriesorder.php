...
class Accessoriesorder extends CI_Controller {
    ...
    public function index() {
        ...
        $Data['OrderType'] = $invoice->getAccessoriesOrderCode()[0];
        $Data['DealerCode'] = $invoice->getDealerCode()[0];
        $Data['OrderNo'] = $invoice->getOrderNumber($Data['OrderType']);
        $Data['Part'] = $this->parts('Accessories Order');
        $this->load->view('header_parts', $Data);
        $this->load->view('accessoriesorder', $Data);
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->AccessoriesOrderReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->AccessoriesOrderReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->AccessoriesOrderReport(NULL, $Brand);
                }
            }
...
//        echo json_encode($fillPartCombo);
    ...
    function allAccessoriesOrders() {
        $invoice = new Parts_invoices();
        $DailyOrder = $invoice->allAccessoriesOrders();
        echo json_encode($DailyOrder);
    }

    function saveAccessoriesOrder() {
        $invoice = new Parts_invoices();
        $savePlanOrder = $invoice->saveAccessoriesOrder();
        redirect(base_url() . "/index.php/Accessoriesorder/index");
    }
    ...
    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneAccessoriesOrder($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteDailyOrder = $invoice->deleteAccessoriesOrder($idOrderNumber["idOrderNumber"]);
            if ($deleteDailyOrder) {
