...
scheduler._temp_matrix_scope = function() {
    function e() {
        for (var e = scheduler.get_visible_events(), t = [], r = 0; r < this.y_unit.length; r++)
            t[r] = [];
        t[s] || (t[s] = []);
        for (var r = 0; r < e.length; r++) {
            for (var s = this.order[e[r][this.y_property]], a = 0; this._trace_x[a + 1] && e[r].start_date >= this._trace_x[a + 1]; )
                a++;
            for (; this._trace_x[a] && e[r].end_date > this._trace_x[a]; )
                t[s][a] || (t[s][a] = []), t[s][a].push(e[r]), a++
        }
        return t
    }
    function t(e, t, r) {
        ...
        if (e && "cell" != this.render) {
            ...
            for (var s = [], a = e.length, n = 0; a > n; n++) {
                ...
                for (this.round_position ? r.apply(this, [i.end_date, !0]) : i.end_date; s.length; ) {
                    var l = s[s.length - 1];
                    if (!(l.end_date.valueOf() <= d.valueOf()))
                        break;
                ...
                if (s.length && (s[s.length - 1]._inner = !0), !_)
                    if (s.length)
                        if (s.length <= s[s.length - 1]._sorder) {
                            ...
                            for (var f = s[0]._sorder, g = 1; g < s.length; g++)
                                s[g]._sorder > f && (f = s[g]._sorder);
            ...
            for (var p = 0; p < e.length; p++)
                e[p]._count = s.max_count;
            for (var m = 0; a > m; m++)
                t += scheduler.render_timeline_event.call(this, e[m], !1)
        }
        return t
    }
    function a(t) {
        var r = "<table style='table-layout:fixed;' cellspacing='0' cellpadding='0'>", a = [];
        if (scheduler._load_mode && scheduler._load(), "cell" == this.render)
            a = e.call(this);
        else
        ...
        for (var u = 0, v = 0; v < scheduler._cols.length; v++)
            u += scheduler._cols[v];
        ...
        for (var v = 0; v < this.y_unit.length; v++) {
            ...
            if (r += "<tr class='" + m.tr_className + "' style='" + m.style_height + "'><td class='" + m.td_className + "' style='" + m.style_width + " height:" + (m.height - 1) + "px;'>" + m.td_content + "</td>", "cell" == this.render)
                for (var d = 0; d < scheduler._cols.length; d++)
                    r += scheduler._ignores[d] ? "<td></td>" : "<td class='dhx_matrix_cell " + scheduler.templates[this.name + "_cell_class"](a[v][d], this._trace_x[d], this.y_unit[v]) + "' style='width:" + (scheduler._cols[d] - 1) + "px'><div style='width:" + (scheduler._cols[d] - 1) + "px'>" + scheduler.templates[this.name + "_cell_value"](a[v][d], this._trace_x[d], this.y_unit[v]) + "</div></td>";
            else {
                ...
                for (var d = 0; d < scheduler._cols.length; d++)
                    r += scheduler._ignores[d] ? "<td></td>" : "<td class='dhx_matrix_cell " + scheduler.templates[this.name + "_cell_class"](a[v], this._trace_x[d], this.y_unit[v]) + "' style='width:" + (scheduler._cols[d] - 1) + "px'><div style='width:" + (scheduler._cols[d] - 1) + "px'></div></td>";
                r += "</table>", r += "</div></td>"
            }
            r += "</tr>"
        }
        r += "</table>", this._matrix = a, t.innerHTML = r, scheduler._rendered = [];
        for (var b = scheduler._obj.getElementsByTagName("DIV"), v = 0; v < b.length; v++)
            b[v].getAttribute("event_id") && scheduler._rendered.push(b[v]);
        this._scales = {};
    ...
    function n(e) {
        ...
        for (var o = _ - scheduler._ignores_detected, h = 0; _ > h; h++)
            this._trace_x[h] = new Date(l), l = scheduler.date.add(l, this.x_step, this.x_unit), scheduler._ignores[h] ? (scheduler._cols[h] = 0, o++) : scheduler._cols[h] = Math.floor(s / (o - h)), s -= scheduler._cols[h], a[h + 1] = a[h] + scheduler._cols[h];
    ...
    function c(e) {
        e = e || event;
        for (var t = e.target ? e.target : e.srcElement; t && "DIV" != t.tagName; )
            t = t.parentNode;
        if (t && "DIV" == t.tagName) {
            var r = t.className.split(" ")[0];
            if ("dhx_scale_bar" == r)
                return{x: o(t), y: -1, src: t, scale: !0}
        }
    }
    scheduler.matrix = {}, scheduler._merge = function(e, t) {
        ...
        scheduler._skin_init(), scheduler._merge(e, {section_autoheight: !0, name: "matrix", x: "time", y: "time", x_step: 1, x_unit: "hour", y_unit: "day", y_step: 1, x_start: 0, x_size: 24, y_start: 0, y_size: 7, render: "cell", dx: 200, dy: 50, event_dy: scheduler.xy.bar_height - 5, event_min_dy: scheduler.xy.bar_height - 5, resize_events: !0, fit_events: !0, show_unassigned: !1, second_scale: !1, round_position: !1, _logic: function(e, t, r) {
                var s = {};
        ...
        scheduler.render_data = function(r, s) {
            if (this._mode != e.name)
                return t.apply(this, arguments);
            if (s && !e.show_unassigned && "cell" != e.render)
                for (var a = 0; a < r.length; a++)
                    this.clear_event(r[a]), this.render_timeline_event.call(this.matrix[this._mode], r[a], !0);
            else
                scheduler._renderMatrix.call(e, !0, !0)
            ...
            if ("minute" == e.x_unit || "hour" == e.x_unit) {
                ...
                if (n * a % 1440)
                    if (+scheduler.date.date_part(new Date(t)) == +scheduler.date.date_part(new Date(s)))
                        e.x_start += r * a;
                    else {
        ...
        scheduler["mouse_" + e.name] = function(t) {
            ...
            if (0 === d && this._ignores[0])
                for (d = 1, a = 0; this._ignores[d]; )
                    d++;
            else if (d == this._cols.length && this._ignores[d - 1]) {
                for (d = this._cols.length - 1, a = 0; this._ignores[d]; )
                    d--;
                d++
            }
            t.x = 0, t.force_redraw = !0, t.custom = !0;
            var _;
            if (d >= e._trace_x.length)
                _ = scheduler.date.add(e._trace_x[e._trace_x.length - 1], e.x_step, e.x_unit), e._end_correction && (_ = new Date(_ - e._end_correction));
            else {
            ...
            if ("move" == this._drag_mode && this._drag_id && this._drag_event) {
                ...
                if (t._ignores = this._ignores_detected || e._start_correction || e._end_correction, c._move_delta || (c._move_delta = (s.start_date - _) / 6e4, this.config.preserve_length && t._ignores && (c._move_delta = this._get_real_event_length(s.start_date, _, e), c._event_length = this._get_real_event_length(s.start_date, s.end_date, e))), this.config.preserve_length && t._ignores) {
                    ...
                    _ = scheduler.date.add(_, c._move_delta, "minute")
            }
            if ("resize" == this._drag_mode && s && (this._drag_from_start && +_ > +s.end_date ? this._drag_from_start = !1 : !this._drag_from_start && +_ < +s.start_date && (this._drag_from_start = !0), t.resize_from_start = this._drag_from_start), e.round_position)
    ...
    scheduler._render_marked_timespan = function(e, r, s, a, n) {
        ...
        if (scheduler.matrix && scheduler.matrix[scheduler._mode]) {
            ...
            if (s)
                ...
                var c = i.order;
                if (o)
                    c.hasOwnProperty(o) && (l.push(o), _.push(i._scales[o]));
                else
                    for (var h in c)
                        c.hasOwnProperty(h) && (l.push(h), _.push(i._scales[h]))
            }
            ...
            if (e.days > 6) {
                ...
                u.push.apply(u, scheduler._get_dates_by_index(e.days));
            for (var g = e.zones, p = scheduler._get_css_classes_by_config(e), m = 0; m < l.length; m++) {
                r = _[m], s = l[m];
                for (var y = 0; y < u.length; y++)
                    for (var x = u[y], b = 0; b < g.length; b += 2) {
    ...
    scheduler._append_mark_now = function(e, t) {
        if (scheduler.matrix && scheduler.matrix[scheduler._mode]) {
            var r = scheduler._currentDate(), s = scheduler._get_zone_minutes(r), a = {days: +scheduler.date.date_part(r), zones: [s, s + 1], css: "dhx_matrix_now_time", type: "dhx_now_time"};
        ...
        for (var r = 0, s = e._trace_x; r < s.length - 1 && + t >= + s[r + 1]; )
            r++;
        return r
        ...
        for (var e in scheduler.matrix) {
            ...
            for (var e = 0; e < t.y_unit.length; e++)
                t.order[t.y_unit[e].key] = e;
