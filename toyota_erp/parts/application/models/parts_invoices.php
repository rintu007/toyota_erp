...
class Parts_invoices extends CI_Model {
    ...
    function allUrgentOrders() {
        $AllDailyOrder = $this->db->select('*')->from('viewurgentorderdetail')->order_by('Date', 'ASC')->get();
...
//        $OrderNumbr = str_replace('-', "", $OrderNum);
        $pos = strrpos($OrderNo, '-');
...
//        $AllDailyOrder = $this->db->select('*')->from('viewmpackageorder')->order_by('Date', 'ASC')->get();
        $AllDailyOrder = $this->db->select('*')->from('viewmpackageorder')->Where('Date', date('Y-m-d'))->get();
        return $AllDailyOrder->result_array();
    }
...
//        $AllDailyOrder = $this->db->select('*')->from('viewdpackageorder')->order_by('Date', 'ASC')->get();
        $AllDailyOrder = $this->db->select('*')->from('viewdpackageorder')->Where('Date', date('Y-m-d'))->get();
        return $AllDailyOrder->result_array();
    }
...
//        $Month = str_replace('0', '', $month_only);
    ...
    function getAccessoriesOrderCode() {
        ...
        $AllDailyOrder = $this->db->select('*')->from('viewaccessoriesorder')->order_by('Date', 'DESC')->get();
    //    $AllDailyOrder = $this->db->select('*')->from('viewaccessoriesorder')->Where('Date', date('Y-m-d'))->get();
        return $AllDailyOrder->result_array();
    }
...
$Year = date('Y', $time);
    ...
    function oneAccessoriesOrder($OrderNumber) {
        $OneDailyOrder = $this->db->select('*')->from('viewaccessoriesorder')->where('OrderNumber', $OrderNumber)->get();
    ...
    function allSpecialOffers() {
        $AllDailyOrder = $this->db->select('*')->from('viewspecialoffer')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//        $OrderNumbr = str_replace('-', "", $OrderNum);
    ...
    function oneSpecialOffer($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewspecialoffer')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
    ...
    function allPlanOrders() {
        $AllDailyOrder = $this->db->select('*')->from('viewplanorder')->order_by('Date', 'DESC')->get();
    //    $AllDailyOrder = $this->db->select('*')->from('viewplanorder')->Where('Date', date('Y-m-d'))->get();
        return $AllDailyOrder->result_array();
    }
...
//        $OrderNumbr = str_replace('-', "", $OrderNum);
    ...
    function onePlanOrder($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewplanorder')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
    ...
    function getBySeaOrderCode() {
     ...
     function oneBySeaOrder($OrderNumber) {
        $OneBySeaOrder = $this->db->select('*')->from('viewbyseaorder')->where('OrderNumber', $OrderNumber)->get();
        return $OneBySeaOrder->result_array();
    }

    function allBySeaOrders() {
        $AllDailyOrder = $this->db->select('*')->from('viewbyseaorder')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//        $AllDailyOrder = $this->db->select('*')->from('viewdailyorder')->order_by('Date', 'ASC')->get();
//        $AllDailyOrder = $this->db->select('*')->from('viewdailyorder')->Where('Date', date('Y-m-d'))->get();
		$AllDailyOrder = $this->db->select('*')->from('viewdailyorder')->order_by('Date', 'Desc')->get();
        return $AllDailyOrder->result_array();
    }
...
//Data For Order_Number Table..
    ...
    function oneDailyOrder($idOrder) {
        $OneDailyOrder = $this->db->select('*')->from('viewdailyorder')->where('OrderNumber', $idOrder)->get();
        return $OneDailyOrder->result_array();
    }
	
	function oneUrgentOrderDetail($idOrder) {
        $OneUrgentOrder = $this->db->select('*')->from('viewurgentorderdetail')->where('OrderNumber', $idOrder)->get();
        return $OneUrgentOrder->result_array();
    }
...
//************ Dhamaka Package *************//
    ...
    function allDhamakaPackage() {
        $AllDailyOrder = $this->db->select('*')->from('viewdhamakapackage')->order_by('Date', 'DESC')->get();
...
//        $OrderNumbr = str_replace('-', "", $OrderNum);
        ...
        $DealerId = $this->getDealer($cookieData['Dealer']);
        $idDealer = $DealerId->IdSubDealer;
        $TypeId = $this->getTypeId('Dhamaka Package');
        $idType = $TypeId->id;
        $Date = $this->input->post('Date');
...
//        $idDispatch = $DispatchMode;
    ...
    function oneDhamakaPackage($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewdhamakapackage')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
...
//        $AllDailyOrder = $this->db->select('*')->from('viewmaintenancepackage')->order_by('Date', 'ASC')->get();
        $AllDailyOrder = $this->db->select('*')->from('viewmaintenancepackage')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//        $OrderNumbr = str_replace('-', "", $OrderNum);
        ...
        $DealerId = $this->getDealer($cookieData['Dealer']);
        $idDealer = $DealerId->IdSubDealer;
        $TypeId = $this->getTypeId('Maintenance Package');
        $idType = $TypeId->id;
        $Date = $this->input->post('Date');
...
$Year = date('Y', $time);
    ...
    function oneMaintenancePackage($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewmaintenancepackage')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
...
//        $AllDailyOrder = $this->db->select('*')->from('viewwarrantyorder')->order_by('Date', 'ASC')->get();
        $AllDailyOrder = $this->db->select('*')->from('viewwarrantyorder')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
$Year = date('Y', $time);
    ...
    function oneWarrantyOrder($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewwarrantyorder')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
...
//************ TGMO *************//
    ...
    function allTgmo() {
        $AllDailyOrder = $this->db->select('*')->from('viewtgmo')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
$Year = date('Y', $time);
    ...
    function oneTgmo($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewtgmo')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
...
//************ Chemical Order *************//
    ...
    function oneChemicalOrder($OrderNumber) {
        $OneChemicalOrder = $this->db->select('*')->from('viewchemicalorder')->where('OrderNumber', $OrderNumber)->get();
        return $OneChemicalOrder->result_array();
    }

    function allChemicalOrder() {
        $AllDailyOrder = $this->db->select('*')->from('viewchemicalorder')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//************ VOR Package *************//
    ...
    function allVOR() {
        $AllDailyOrder = $this->db->select('*')->from('viewvor')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//        $Month = str_replace('0', '',$month_only);
    ...
    function oneVOR($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewvor')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
...
//************ Dhamaka Package *************//
    function getLocalPurchaseCode() {
        $invoiceType = $this->db->select('*')->from('invoice_claim_type')->where('Title', 'Local Package')->get();
        return $invoiceType->result_array();
    }

    function allLocalPurchase() {
        $AllDailyOrder = $this->db->select('*')->from('viewlocalpurchase')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//Data For Order_Number Table..
    ...
    function oneLocalPurchase($OrderNumber) {
        $OneDhamakaPackage = $this->db->select('*')->from('viewlocalpurchase')->where('OrderNumber', $OrderNumber)->get();
        return $OneDhamakaPackage->result_array();
    }
...
//************ Other  *************//
    ...
    function allOtherPurchase() {
        $AllDailyOrder = $this->db->select('*')->from('viewotherpurchase')->order_by('Date', 'DESC')->get();
        return $AllDailyOrder->result_array();
    }
...
//                . " WHERE vgo.`Month` = EXTRACT(MONTH FROM NOW()) AND vgo.`OrderType` = '$OrderType'");
        ...
        $MaxNumber = $this->db->query("SELECT max(cast(order_number.Number as DECIMAL))+1 as Number FROM order_number
                ...
                WHERE order_number.BrandCode = '" . $this->session->userdata("BrandCode") . "' AND order_number.`Month` = EXTRACT(MONTH FROM NOW()) AND order_number.`Year` = EXTRACT(YEAR FROM NOW()) AND invoice_claim_type.Title='$OrderType'");
        return $MaxNumber->result_array()[0];
    }
...
//        echo $this->db->last_query();exit();
	...
	function getMonthlyOrderCode() {
        $invoiceType = $this->db->select('*')->from('invoice_claim_type')->where('Title', 'MONTHLY ORDER CYCLE')->get();
        return $invoiceType->result_array();
    }
