...
class Dpackageorder extends CI_Controller {
    ...
    public function index() {
        ...
        $Data['OrderType'] = $invoice->getDPackageOrderCode()[0];
        ...
        $this->load->view('dpackageorder', $Data);
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->DPackageOrderReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->DPackageOrderReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->DPackageOrderReport(NULL, $Brand);
                }
            }
//            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
            ...
            $pdf->SetTitle('D Package Order Reports');
            $pdf->SetSubject('');
            $pdf->SetKeywords('D Package Order Report');

            //set default header data
//        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Parts Division', "D Package Order Report");
...
//        $pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
    ...
    function allDPackageOrders() {
        $invoice = new Parts_invoices();
        $DailyOrder = $invoice->allDPackageOrders();
        echo json_encode($DailyOrder);
    }

    function saveDPackageOrder() {
        $invoice = new Parts_invoices();
        $savePlanOrder = $invoice->saveDPackageOrder();
    }
    ...
    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneDPackageOrder($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteDailyOrder = $invoice->deleteDPackageOrder($idOrderNumber["idOrderNumber"]);
            if ($deleteDailyOrder) {
    ...
    function orderNumber() {
        $invoice = new Parts_invoices();
        $orderNumber = $invoice->getOrderNumber($invoice->getDPackageOrderCode()[0]);
        return $orderNumber;
    }
