...
//                        }
                    ...
                    <div>
                        <table id="mp" title="M-Package Orders" style="width:auto;height:auto;"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th field="PartNumber" width="60" editor="{type:'combobox',options:{valueField:'PartNumber', textField:'PartNumber', url:'<?= base_url() ?>/index.php/invoices/parts/<?= 'M Package Order' ?>',required:true}}">Part Number</th>
                        ...
                        <div id="toolbar">
                            ...
                            <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="javascript:$('#mp').edatagrid('addRow');
                            <?php
                            ...
                            <a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="javascript:$('#mp').edatagrid('destroyRow');
                                    javascript:$('#mp').edatagrid('reload');">Delete</a>
                            <a href="#" id="saveURL" class="easyui-linkbutton" iconCls="icon-save" plain="true" onclick="javascript:$('#mp').edatagrid('saveRow')">Save</a>
                            <a href="#" class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="javascript:$('#mp').edatagrid('cancelRow')">Cancel</a>
                        </div>
                    </div>
                </fieldset>
            </form>
...
<script>
    ...
    $('#mp').edatagrid({
        url: '<?= base_url() ?>/index.php/mpackageorder/allMPackageOrders',
        saveUrl: '<?= base_url() ?>/index.php/mpackageorder/saveMPackageOrder',
        updateUrl: '<?= base_url() ?>/index.php/parts/edit',
        destroyUrl: '<?= base_url() ?>/index.php/mpackageorder/delete',
    ...
    function destroyUser() {
        var row = $('#mp').edatagrid('getSelections');
        $.each(row, function (index, value) {
            if (value) {
                $.messager.confirm('Confirm', 'Are you sure you want to delete this order?', function (r) {
                    if (r) {
                        $.post('<?= base_url() ?>/index.php/mpackageorder/delete', {id: value.OrderNumber}, function (result) {
                            window.res = result;
                            if (result.success) {
                                $('#mp').edatagrid('reload');    // reload the user data
