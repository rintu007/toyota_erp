...
class Dhamakapackage extends CI_Controller {
    ...
    public function index() {
        ...
        $this->session->set_userdata('BrandCode', $BrandCode);
        ...
        $Data['Part'] = $this->parts("Dhamaka Package");
        $this->load->view('header_parts', $Data);
        $this->load->view('Dhamakapackage', $Data);
    ...
    public function report() {
        if (isset($_POST['pdf'])) {
            ...
            if ($Filter == "All") {
                $Data = $Reporting->DhamakaPackageReport(NULL, NULL);
            } else if ($Filter == "OrderNumber") {
                if ($OrderNumber != "") {
                    $Data = $Reporting->DhamakaPackageReport($OrderNumber, NULL);
                }
            } else {
                if ($Brand != "Select Brand") {
                    $Data = $Reporting->DhamakaPackageReport(NULL, $Brand);
                }
            }
//            $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
            ...
            $pdf->SetTitle('Accessories Order Reports');
            $pdf->SetSubject('');
            $pdf->SetKeywords('Accessories Order Report');

            //set default header data
//        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
            $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, 'Parts Division', "Accessories Order Report");
...
//        $pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
    ...
    function view() {
        ...
        $Data['AllDhamkapackages'] = $invoice->allDhamakaPackage();
        $this->load->view('header_parts', $Data);
...
//        echo json_encode($fillPartCombo);
    ...
    function Dhamkapackage($idInvoice) {
        $Data = array();
        $invoice = new Parts_invoices();
        $Data['OrderType'] = $invoice->getDhamakaPackageCode()[0];
        $Data['DealerCode'] = $invoice->getDealerCode()[0];
        ...
        $this->load->view('header_parts', $Data);
        $this->load->view('Dhamkapackage', $Data);
        $this->load->view('footer');
    }

    function saveDhamakapackage() {
        $invoice = new Parts_invoices();
        $saveDhamkapackage = $invoice->saveDhamakaPackage();
        redirect(base_url() . "/index.php/dhamakapackage/index");
    }

    function allDhamkapackage() {
        $invoice = new Parts_invoices();
        $Dhamkapackage = $invoice->allDhamakaPackage();
        echo json_encode($Dhamkapackage);
    }

    function delete() {
        $invoice = new Parts_invoices();
        $idOrderNumber = $invoice->oneDhamakaPackage($_POST["id"])[0];
        if (!empty($idOrderNumber)) {
            $deleteDhamakaPackage = $invoice->deleteDhamakaPackage($idOrderNumber["idOrderNumber"]);
            if ($deleteDhamakaPackage) {
