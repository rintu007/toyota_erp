        ...
        <div class="right-pnel">
            <form name="myform" onSubmit="return validationform()" method="post"
                ...
                <fieldset>
                    ...
                    <div>
                        <table id="po" title="By Sea Orders" style="width:auto;height:auto;"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th field="PartNumber" width="60" editor="{type:'combobox',options:{valueField:'PartNumber', textField:'PartNumber', url:'<?= base_url() ?>/index.php/invoices/parts/<?= "By Sea Order" ?>',required:true}}">Part Number</th>
                                    ...
                                    <th field="BySeaPrice" width="50" editor="{type:'validatebox',options:{required:false}}">By Sea Price</th>
                        ...
                        <div id="toolbar">
                            ...
                            <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="javascript:$('#po').edatagrid('addRow');
                            <?php
...
//                                    $('div.datagrid-cell-c1-Dispatch-Mode').children('table').children('tbody').children('tr').children('td').children('span').children('.combo-value').val('By Sea');
//                                    $('div.datagrid-cell-c1-Dispatch-Mode').children('table').children('tbody').children('tr').children('td').children('span').children('.combo-text').val('By Sea');
                               ">New</a>
                            <a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="javascript:$('#po').edatagrid('destroyRow');
                                    javascript:$('#po').edatagrid('reload');">Delete</a>
                            <a href="#" id="saveURL" class="easyui-linkbutton" iconCls="icon-save" plain="true" onclick="javascript:$('#po').edatagrid('saveRow')">Save</a>
                            <a href="#" class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="javascript:$('#po').edatagrid('cancelRow')">Cancel</a>
                        </div>
                    </div>
                </fieldset>
            </form>
...
<script>
    ...
    $('#po').edatagrid({
        url: '<?= base_url() ?>/index.php/byseaorder/allBySeaOrders',
        saveUrl: '<?= base_url() ?>/index.php/byseaorder/saveBySeaOrder',
        updateUrl: '<?= base_url() ?>/index.php/parts/edit',
        destroyUrl: '<?= base_url() ?>/index.php/byseaorder/delete',
...
//                        $('div.datagrid-cell-c1-Dispatch-Mode').children('table').children('tbody').children('tr').children('td').children('span').children('.combo-text').val('By Road');
    ...
    function destroyUser() {
        var row = $('#po').edatagrid('getSelections');
        $.each(row, function (index, value) {
            if (value) {
                $.messager.confirm('Confirm', 'Are you sure you want to delete this order?', function (r) {
                    if (r) {
                        $.post('<?= base_url() ?>/index.php/byseaorder/delete', {id: value.OrderNumber}, function (result) {
                            window.res = result;
                            if (result.success) {
