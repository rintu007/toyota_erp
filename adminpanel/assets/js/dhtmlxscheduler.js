...
window.dhtmlXScheduler = window.scheduler = {version: "4.1.0"}, window.dhtmlx || (dhtmlx = function(e) {
    ...
    for (var t = {}, i = 0; i < e.attributes.length; i++)
        t[e.attributes[i].name] = e.attributes[i].value;loc
    ...
    if (window.name) {
        for (var t = parent.frames[window.name].frameElement.offsetParent, i = 0, s = 0; t; )
            i += t.offsetLeft, s += t.offsetTop, t = t.offsetParent;
    ...
    if (_isIE)
        return t || (t = this.xmlDoc.nodeName ? this.xmlDoc : this.xmlDoc.responseXML), t || dhtmlxError.throwError("LoadXML", "Incorrect XML", [t || this.xmlDoc, this.mainObject]), i && t.setProperty("SelectionNamespaces", "xmlns:xsl='" + i + "'"), "single" == s ? t.selectSingleNode(e) : t.selectNodes(e) || new Array(0);
    var n = t;
    ...
    for (n; n < i.length; n++)
        s = this._getAllNamedChilds(s, i[n]);
    ...
    for (var s = 0; s < e.length; s++)
        for (var n = 0; n < e[s].childNodes.length; n++)
            _isKHTML ? e[s].childNodes[n].tagName && e[s].childNodes[n].tagName.toUpperCase() == t && (i[i.length] = e[s].childNodes[n]) : e[s].childNodes[n].tagName == t && (i[i.length] = e[s].childNodes[n]);
    return i
    ...
    function _(e, t, i, s) {
        return"object" != typeof e && (e = {text: e, type: t, expire: i, id: s}), e.id = e.id || u.uid(), e.expire = e.expire || u.expire, e
    }
    var c = null;
    ...
    u.seed = (new Date).valueOf(), u.uid = function() {
        ...
        if (n && (s = this.styles[n], t = !0), this.callEvent("onRowMark", [e, t, i, n]) && (s = this.styles[t ? i : "clear"] + s, this.obj[this._methods[0]](e, s), n && n.details)) {
            ...
            for (var r = 0; r < n.details.length; r++)
                n.details[r] && this.obj[this._methods[1]](e, r, s)
        }
    }, getState: function(e) {
        ...
        i && (t = {value: t, details: i, toString: function() {
        ...
        var i = [], s = [];
        for (var n in e)
            e.hasOwnProperty(n) && (i.push(this.serialize_one(e[n], n + this.post_delim)), s.push(n));
        ...
        var i = [];
        for (var s in e)
            e.hasOwnProperty(s) && i.push(this.escape((t || "") + s) + "=" + this.escape(e[s]));
        return i.join("&")
        ...
        if (r || this.set_invalid(e, i), this._uActions && this._uActions[i] && !this._uActions[i](s))
    ...
    var e = function(e) {
        ...
        for (var n = 0; n < i.length; n++)
            i[n] = "dhx_filter[" + i[n] + "]=" + encodeURIComponent(s[n]);
    ...
    if (!this._colls_loaded && this.cellType) {
        for (var t = [], i = 0; i < this.cellType.length; i++)
            (0 === this.cellType[i].indexOf("co") || 2 == this._con_f_used[i]) && t.push(i);
    ...
    if (this._drag_mode) {
        ...
        if (!this._drag_pos || t.force_redraw || this._drag_pos.x != t.x || this._drag_pos.y != t.y) {
            ...
            if ("move" == this._drag_mode)
                i = this._min_date.valueOf() + 6e4 * (t.y * this.config.time_step + 24 * t.x * 60 - (scheduler._move_pos_shift || 0)), !t.custom && this._table_view && (i += 1e3 * this.date.time_part(o.start_date)), i = this._correct_shift(i), t._ignores && this.config.preserve_length && this._table_view ? (this.matrix && (d = this.matrix[this._mode]), d = d || {x_step: 1, x_unit: "day"}, s = 1 * i + this._get_fictional_event_length(i, this._drag_event._event_length, d)) : s = o.end_date.valueOf() - (o.start_date.valueOf() - i);
            else {
                if (i = o.start_date.valueOf(), s = o.end_date.valueOf(), this._table_view) {
                    ...
                    s = this.date.date_part(new Date(o.end_date.valueOf() - 1)).valueOf() + t.y * this.config.time_step * 6e4, this._els.dhx_cal_data[0].style.cursor = "s-resize", ("week" == this._mode || "day" == this._mode) && (s = this._correct_shift(s));
            ...
            if (!this._table_view && (t.x != this._get_event_sday({start_date: new Date(s), end_date: new Date(s)}) || new Date(s).getHours() >= this.config.last_hour)) {
                ...
                if (this.callEvent("onBeforeEventChanged", [t, e, i, this._drag_event])) {
                    ...
                    if (this._drag_id = this._drag_mode = null, i && this.config.edit_on_create) {
                        if (this.unselect(), this._new_event = new Date, this._table_view || this.config.details_on_create || !this.config.select)
                            return scheduler.callEvent("onDragEnd", [s, n, e]), this.showLightbox(s);
                        this._drag_pos = !0, this._select_id = this._edit_id = s
                    } else
                        this._new_event || this.callEvent(i ? "onEventAdded" : "onEventChanged", [s, this.getEvent(s)])
                } else
                    i ? this.deleteEvent(t.id, !0) : (this._drag_event._dhx_changed = !1, this._clear_temporary_properties(t, this._drag_event), scheduler._lame_copy(t, this._drag_event), this.updateEvent(t.id))
            }
    ...
    for (var r = n - this._ignores_detected, a = 0; n > a; a++)
        this._ignores[a] ? (this._cols[a] = 0, r++) : this._cols[a] = Math.floor(s / (r - a)), s -= this._cols[a], this._colsS[a] = (this._cols[a - 1] || 0) + (this._colsS[a - 1] || (this._table_view ? 0 : this.xy.scale_width + 2)), this._colsS.col_length = n;
    ...
    if (this.templates[this._mode + "_date"]) {
        ...
        if (n = r = this._table_view ? scheduler.date.week_start(a) : a, this._min_date = n, this._els.dhx_cal_date[0].innerHTML = this.templates[this._mode + "_date"](a, d, this._mode), this._max_date = d, scheduler._render_scales(e, t), this._table_view)
            this._reset_month_scale(t, a, r);
    ...
    if (r)
        for (var a = new Date(e), d = 0; t > d; d++)
            r(a) && (this._ignores_detected += 1, this._ignores[d] = !0, n && t++), a = scheduler.date.add(a, s, i)
    ...
    if (e && "object" == typeof e) {
        for (n = {}, s = [Array, Date, Number, String, Boolean], i = 0; i < s.length; i++)
            e instanceof s[i] && (n = i ? new s[i](e) : new s[i]);
        t.push(e, n);
        for (i in e)
            Object.prototype.hasOwnProperty.apply(e, [i]) && (n[i] = scheduler._lame_clone(e[i], t))
    }
    return n || e
    ...
    return null !== e.lightbox_id && void 0 !== e.lightbox_id
        ...
        for (var i = "var temp=date.match(/[a-zA-Z]+|[0-9]+/g);", s = e.match(/%[a-zA-Z]/g), n = 0; n < s.length; n++)
            switch (s[n]) {
                case"%j":
                case"%d":
                    i += "set[2]=temp[" + n + "]||1;";
    ...
    }}, scheduler.locale = {date: {month_full: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"], month_short: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], day_full: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], day_short: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]}, labels: {dhx_cal_today_button: "Today", day_tab: "Day", week_tab: "Week", month_tab: "Month", new_event: "New Appointment For Service", icon_save: "Schedule", icon_cancel: "Cancel", icon_details: "Details", icon_edit: "Edit", icon_delete: "Delete", confirm_closing: "", confirm_deleting: "Scheduled Service  will be deleted permanently, are you sure?", section_description: "Description", section_time: "Time period", full_day: "Full day", confirm_recurring: "Do you want to edit the whole set of repeated events?", section_recurring: "Repeat event", button_recurring: "Disabled", button_recurring_open: "Enabled", button_edit_series: "Edit series", button_edit_occurrence: "Edit occurrence", agenda_tab: "Agenda", date: "Date", description: "Description", year_tab: "Year", week_agenda_tab: "Agenda", grid_tab: "Grid", drag_to_create: "Drag to create", drag_to_move: "Drag to move", message_ok: "OK", message_cancel: "Cancel"}}, scheduler.config = {default_date: "%j %M %Y", month_date: "%F %Y", load_date: "%Y-%m-%d", week_date: "%l", day_date: "%D, %F %j", hour_date: "%H:%i", month_day: "%d", xml_date: "%m/%d/%Y %H:%i", api_date: "%d-%m-%Y %H:%i", preserve_length: !0, time_step: 5, start_on_monday: 1, first_hour: 0, last_hour: 24, readonly: !1, drag_resize: 1, drag_move: 1, drag_create: 1, dblclick_create: 1, edit_on_create: 1, details_on_create: 0, resize_month_events: !1, resize_month_timed: !1, cascade_event_display: !1, cascade_event_count: 4, cascade_event_margin: 30, multi_day: !0, multi_day_height_limit: 0, drag_lightbox: !0, preserve_scroll: !0, select: !0, server_utc: !1, touch: !0, touch_tip: !0, touch_drag: 500, quick_info_detached: !0, positive_closing: !1, drag_highlight: !0, limit_drag_out: !1, icons_edit: ["icon_save", "icon_cancel"], icons_select: ["icon_details", "icon_edit", "icon_delete"], buttons_left: ["dhx_save_btn", "dhx_cancel_btn"], buttons_right: ["dhx_delete_btn"], lightbox: {sections: [{name: "description", height: 200, map_to: "text", type: "textarea", focus: !0}, {name: "regno", map_to: "text", type: "text"}, {name: "time", height: 72, type: "time", map_to: "auto"}]}, highlight_displayed_event: !0, left_border: !1}, scheduler.templates = {}, scheduler.init_templates = function() {
    ...
    s(scheduler.templates, {day_date: t(i.default_date), month_date: t(i.month_date), week_date: function(e, t) {
    ...
    for (var e = ["text", "Text", "start_date", "StartDate", "end_date", "EndDate"], t = function(e) {
        ...
        scheduler["getEvent" + e[s + 1]] = t(e[s]), scheduler["setEvent" + e[s + 1]] = i(e[s])
    ...
    if (this.config.multi_day && !this._table_view) {
        for (var n = [], r = [], i = 0; i < e.length; i++)
            this._is_main_area_event(e[i]) ? n.push(e[i]) : r.push(e[i]);
    ...
    if (!t) {
        for (var n = 0; n < e.length; n++)
            e[n]._count = i[e[n]._sday].max_count;
        for (var n = 0; n < s.length; n++)
            s[n]._count = i[s[n]._sday].max_count
    }
    return e
    ...
    for (var i, s = [], n = [[], [], [], [], [], [], []], r = this._colsS.heights, a = this._cols.length, d = {}, o = 0; o < e.length; o++) {
        ...
        if (l._sday = f % a, !this._ignores[l._sday] || !l._timed) {
            ...
            for (v = 0; v < m.length && !(m[v]._eday <= l._sday); v++)
                ;
            if (l._sorder && t || (l._sorder = v), l._sday + l._length <= a)
                i = null, s.push(l), m[v] = l, r[l._sweek] = m.length - 1, l._first_chunk = _.first_chunk, l._last_chunk = _.last_chunk;
            else {
    ...
    if (!o && scheduler.renderEvent(v, e, s, n, a, d))
        return v;
    v = l.firstChild;
    ...
    if (e = e || this._load_url) {
        ...
        if (t = t || this._date, this._load_mode) {
            ...
            i = t;
            for (var n = !0; i < this._max_date; )
                i = this.date.add(i, 1, this._load_mode), this._loaded[s(t)] && n ? t = this.date.add(t, 1, this._load_mode) : n = !1;
            var r = i;
            do
                i = r, r = this.date.add(i, -1, this._load_mode);
            while (r > t && this._loaded[s(r)]);
            if (t >= i)
                return!1;
            for (dhtmlxAjax.get(e + "&from=" + s(t) + "&to=" + s(i), function(e) {
                scheduler.on_load(e)
            }); i > t; )
                this._loaded[s(t)] = !0, t = this.date.add(t, 1, this._load_mode)
        } else
    ...
    }, c_start: "BEGIN:VCALENDAR", e_start: "BEGIN:VEVENT", e_end: "END:VEVENT", c_end: "END:VCALENDAR"}, scheduler._lightbox_controls = {}, scheduler.formSection = function(e) {
    var t = this.config.lightbox.sections, i = 0;
    for (i; i < t.length && t[i].name != e; i++)
        ;
    var s = t[i];
    scheduler._lightbox || scheduler.getLightbox();
    var n = document.getElementById(s.id), r = n.nextSibling, a = {section: s, header: n, node: r, getValue: function(e) {
    ...
    return e.control = e.node.getElementsByTagName("select"), e
}, scheduler.form_blocks = {template: {render: function(e) {
            ...
            var t = (e.height || "500") + "px";
            return"<div class='dhx_cal_ltext' style='height:" + t + ";'><textarea id='description'></textarea></div><label style='margin-left: 21px; font-weight: bolder; color: dimgrey;'>Registration No.</label><input type='text' style='margin-left: 14px; width: 606px;' id='regno'/>";
        }, set_value: function(e, t) {
            ...
            for (var t = (e.height || "23") + "px", i = "<div class='dhx_cal_ltext' style='height:" + t + ";'><select id='bay' style='width:100%;'>", s = 0; s < e.options.length; s++)
            ...
            var t = e.firstChild;
            scheduler._focus(t, !0);
        }}, job: {render: function(e) {
            for (var t = (e.height || "23") + "px", i = "<div class='dhx_cal_ltext' style='height:" + t + ";'><select id='jobref' style='width:100%;'>", s = 0; s < e.options.length; s++)
                i += "<option value='" + e.options[s].idJobRef + "'>" + e.options[s].JobTask + "</option>";
            return i += "</select></div>";
        }, set_value: function(e, t, i, s) {
            var n = e.firstChild;
            !n._dhx_onchange && s.onchange && (n.onchange = s.onchange, n._dhx_onchange = !0), "undefined" == typeof t && (t = (n.options[0] || {}).value), n.value = t || ""
            ...
            for (var a = "", d = 0; d < t.length; d++) {
                ...
                switch (d > 0 && (a += " "), o) {
                    case"%Y":
                        e._time_format_order[3] = d, a += "<select id='StartYear'>";
                        for (var l = s.getFullYear() - 5, h = 0; 10 > h; h++)
                            a += "<option value='" + (l + h) + "'>" + (l + h) + "</option>";
                        a += "</select> ";
                        break;
                    case"%m":
                        e._time_format_order[2] = d, a += "<select id='StartMonth'>";
                        for (var h = 0; 12 > h; h++)
                            a += "<option value='" + h + "'>" + this.locale.date.month_full[h] + "</option>";
                        a += "</select>";
                        break;
                    case"%d":
                        e._time_format_order[1] = d, a += "<select id='StartDate' style='display:none'>";
                        for (var h = 1; 32 > h; h++)
                            a += "<option value='" + h + "'>" + h + "</option>";
                        a += "</select>";
                        break;
                    case"%H:%i":
                        e._time_format_order[0] = d, a += "<select id='StartTime' style='margin-left: -172px;'>";
                        var h = r, _ = s.getDate();
...
//                            a += "<option value='" + h + "'>" + c + "</option>", e._time_values.push(h), s.setTime(s.valueOf() + 60 * this.config.time_step * 1e3);
                            a += "<option value='" + h + "'>" + c + "</option>", e._time_values.push(c), s.setTime(s.valueOf() + 60 * this.config.time_step * 1e3);
                            var u = s.getDate() !== _ ? 1 : 0;
                            h = 24 * u * 60 + 60 * s.getHours() + s.getMinutes();
                        }
                        a += "</select>";
//                        e._time_format_order[3] = d, a += "<select>";
//                        a += "<option value='9:00'>9:00</opiton>";
//                        a += "<option value='10:00'>10:00</opiton>";
//                        a += "<option value='11:00'>11:00</opiton>";
//                        a += "<option value='12:00'>12:00</opiton>";
//                        a += "<option value='1:00'>1:00</opiton>";
//                        a += "<option value='2:00'>2:00</opiton>";
//                        a += "<option value='3:00'>3:00</opiton>";
//                        a += "<option value='4:00'>4:00</opiton>";
//                        a += "<option value='5:00'>5:00</opiton>";
//                        a += "<option value='6:00'>6:00</opiton>";
//                        a += "</select> ";
                }
            }
            for (var b = "", d = 0; d < t.length; d++) {
                ...
                switch (d > 0 && (b += " "), o) {
                    case"%Y":
                        e._time_format_order[3] = d, b += "<select id='EndYear'>";
                        for (var l = s.getFullYear() - 5, h = 0; 10 > h; h++)
                            b += "<option value='" + (l + h) + "'>" + (l + h) + "</option>";
                        b += "</select> ";
                        break;
                    case"%m":
                        e._time_format_order[2] = d, b += "<select id='EndMonth'>";
                        for (var h = 0; 12 > h; h++)
                            b += "<option value='" + h + "'>" + this.locale.date.month_full[h] + "</option>";
                        b += "</select>";
                        break;
                    case"%d":
                        e._time_format_order[1] = d, b += "<select id='EndDay' style='display:none'>";
                        for (var h = 1; 32 > h; h++)
                            b += "<option value='" + h + "'>" + h + "</option>";
                        b += "</select>";
                        break;
                    case"%H:%i":
                        e._time_format_order[0] = d, b += "<select id='EndTime'>";
                        var h = r, _ = s.getDate();
                        for (e._time_values = []; n > h; ) {
                            var c = this.templates.time_picker(s);
//                            a += "<option value='" + h + "'>" + c + "</option>", e._time_values.push(h), s.setTime(s.valueOf() + 60 * this.config.time_step * 1e3);
                            b += "<option value='" + h + "'>" + c + "</option>", e._time_values.push(c), s.setTime(s.valueOf() + 60 * this.config.time_step * 1e3);
                            var u = s.getDate() !== _ ? 1 : 0;
                            h = 24 * u * 60 + 60 * s.getHours() + s.getMinutes();
                        }
                        b += "</select>";
...
//                    scheduler.save_lightbox();
                    ...
                    EndYear = $('#EndYear').val();
                    $.ajax({
                        url: 'http://127.0.0.1/dealer001/service/index.php/Jobprogresscontrolboard/Add',
                        type: 'POST',
                        data: {StartMin: StartMin, EndMin: EndMin, EndDate: EndDate, EndMonth: EndMonth, EndYear: EndYear, StartDate: StartDate, StartMonth: StartMonth, StartYear: StartYear, Bay: Bay, StartTime: StartTime, EndTime: EndTime, Description: Description, RegNo: RegNo, JobRef: JobRef, TotalTime: TotalTime},
...
//                            }
    ...
    s(document.body, e[0], function(e) {
        if (!i(e)) {
            ...
            if (d && c && n(d, t(e), 0), d = t(e), _)
                return d ? void((a.target != d.target || Math.abs(a.pageX - d.pageX) > 5 || Math.abs(a.pageY - d.pageY) > 5) && (h = !0, clearTimeout(o))) : void(h = !0)
        }
        ...
        if (!i(e)) {
            var s;
            if (l = h = !1, _ = !0, scheduler._temp_touch_block = !0, s = d = t(e), !s)
                return void(h = !0);
            var n = new Date;
            if (!h && !l && 250 > n - u)
                return scheduler._click.dhx_cal_data(s), window.setTimeout(function() {
    ...
    e._methods = ["_set_event_text_style", "", "changeEventId", "_dp_hook_delete"], this._dp_hook_delete = function(e) {
        ...
        for (var s in t)
            0 !== s.indexOf("_") && (i[s] = t[s] && t[s].getUTCFullYear ? this.obj.templates.xml_format(t[s]) : t[s]);
        return i
            ...
            if (scheduler.templates.month_day = function(i) {
                ...
                for (var i = scheduler._els.dhx_cal_navline[0].getElementsByTagName("div"), s = null, n = 211, r = 0; r < i.length; r++) {
                    ...
                    if (d)
                        ...
                        0 === (a.className || "").indexOf("dhx_minical_icon") && a.parentNode == scheduler._els.dhx_cal_navline[0] && (s = a)
                }
                s && (s.style.left = n + "px")
            }
